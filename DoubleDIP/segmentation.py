...
def segmentation(image_name, image, fore, back, save_path):
    results = []
    for i in range(1):
    ...
    save_image(image_name + "_real_left", clear_image_places * median([best_result.left for best_result in results]), save_path)
    save_image(image_name + "_real_right", image * (1 - clear_image_places) + clear_image_places * median([best_result.right for best_result in results]), save_path)
