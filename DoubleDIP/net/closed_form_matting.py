...
def _rolling_block(A, block=(3, 3)):
    ...
    #print(A.strides) (4096,8)  8^4=4096
    return as_strided(A, shape=shape, strides=strides)


def compute_laplacian(img, mask=None, eps=10**(-5), win_rad=1):
    ...
    #print(win_inds.shape)  (510,510,3,3)
    ...
    if mask is not None:
        ...
        win_mask = cp.sum(mask.ravel()[win_inds], axis=2)
        #print(mask.ravel().shape)  #(512*512=262144,1)
        #print(mask.ravel()[win_inds].shape)  #(510,510,9)
        win_inds = win_inds[win_mask > 0, :]
        #print(win_inds.shape) #(11208,9)
    else:
        win_inds = win_inds.reshape(-1, win_size)

    
    winI = ravelImg[win_inds]  #(11208,9,3)
    #print(winI.shape)
    win_mu = cp.mean(winI, axis=1, keepdims=True)  #(11208,1,3)
    win_var = np.einsum('...ji,...jk ->...ik', winI, winI) / win_size - np.einsum('...ji,...jk ->...ik', win_mu, win_mu)    #(11208,3,3)
    # print(win_mu.shape)
    # print(win_var.shape)
    ...
    vals = (1.0/win_size)*(1 + np.einsum('...ij,...kj->...ik', X, winI - win_mu)) #np.eye(win_size) -
    #print(vals.shape)  #(260100,9,9)
    #nz_indsCol = np.tile(win_inds, win_size).ravel()
    #nz_indsRow = np.repeat(win_inds, win_size).ravel()
    #print(np.tile(win_inds, win_size).shape)
    #print(np.repeat(win_inds, win_size).shape)
    #nz_indsVal = vals.ravel()
    #L = scipy.sparse.coo_matrix((nz_indsVal, (nz_indsRow, nz_indsCol)), shape=(h*w, h*w))
    return vals.ravel()


def compute_alpha(img, mask=None, eps=10 ** (-7), win_rad=1):
    """Computes Matting Laplacian for a given image.
    ...
    if mask is not None:
        ...
        win_mask = cp.sum(mask.ravel()[win_inds], axis=2)
        # print(mask.ravel().shape)
        # print(mask.ravel()[win_inds].shape)
        win_inds = win_inds[win_mask > 0, :]
        # print(win_inds.shape)
    else:
        win_inds = win_inds.reshape(-1, win_size)
    #print(win_inds.shape)  #(510*510,9)
    nz_indsCol = np.tile(win_inds, win_size).ravel()
    nz_indsRow = np.repeat(win_inds, win_size).ravel()
    #print(nz_indsCol) #(0 1 2 ..., 262141  262142  262143)
    #print(nz_indsRow) #(0 0 0 ..., 262143  262143  262143)
    cha = (nz_indsCol - nz_indsRow)**2
    #print(cha.shape) #(510*510*9*9=21068100)
    return cha
...
def closed_form_matting_with_prior(image, prior, prior_confidence, consts_map=None):
    ...
    confidence = scipy.sparse.diags(prior_confidence.ravel())  #多维数组降为一维
