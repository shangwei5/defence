...
import os
from DoubleDIP.net import *
from DoubleDIP.net.losses import StdLoss, YIQGNGCLoss, GradientLoss, ExtendedL1Loss, GrayLoss
from DoubleDIP.net.noise import get_noise, NoiseNet
from DoubleDIP.utils.image_io import *
from DoubleDIP.net.downsampler import *
from skimage.measure import compare_psnr
from cv2.ximgproc import guidedFilter
import cv2
from DoubleDIP.networks import *
# from util import *
import warnings

warnings.filterwarnings("ignore")

SegmentationResult = namedtuple("SegmentationResult", ['left', 'right', 'psnr'])
...
class Segmentation(object):
    def __init__(self, image_name, image, plot_during_training=True,
                 ...
                 bg_hint=None, fg_hint=None, save_path=None,
                 show_every=500,
    ...
    def optimize(self):
        ...
        for j in range(self.second_step_iter_num):
            ...
            optimizer.step()
            if j % 250 == 0:
                model_path_1 = os.path.join(self.save_path, 'model/left_net_%d.pth'%j)
                model_path_2 = os.path.join(self.save_path, 'model/right_net_%d.pth' % j)
                #model_path_3 = os.path.join(self.save_path, 'model/mask_net_%d.pth' % j)
                #os.makedirs(model_path_1,exist_ok=True)
                #os.makedirs(model_path_2, exist_ok=True)
                #torch.save(self.left_net, model_path_1)
                #torch.save(self.right_net, model_path_2)
                #torch.save(self.mask_net, model_path_3)
        self._update_result_closure()
        #if self.plot_during_training:
    ...
    def finalize(self, iteration):
        # save_image(self.image_name + "_%d_left" % iteration, self.best_result.left, self.save_path)
        # save_image(self.image_name + "_%d_learned_mask" % iteration, self.best_result.learned_mask, self.save_path)
        save_image(self.image_name + "_%d_right" % iteration, self.best_result.right, self.save_path)
        #save_image(self.image_name + "_%d_original" % iteration, self.images[0], self.save_path)
        # save_image(self.image_name + "_fg_bg", ((self.fg_hint - self.bg_hint) + 1) / 2)
        #save_image(self.image_name + "_%d_mask" % iteration, self.best_result.mask, self.save_path)

    def _update_result_closure(self):
        self._finalize_iteration()
        #self._fix_mask()
        self.current_result = SegmentationResult(
                                                 left=torch_to_np(self.left_net_outputs[0]),
                                                 right=torch_to_np(self.right_net_outputs[0]),
    ...
    def _fix_mask(self):
        ...
        self.fixed_masks = [to_bin(m) for m in masks_np]
    ...
    def _initialize_any_step(self, iteration):
        ...
        for left_net_original_input, right_net_original_input \
                in zip(self.left_net_inputs, self.right_net_inputs):
    ...
    def _step2_optimize_with_hints(self, iteration):
        ...
        for left_out, right_out, mask_out, original_image_torch in zip(self.left_net_outputs,
                                                                       ...
                                                                       self.fg_hints_torch,
                                                                       self.images_torch):
            # mask_out_c = mask_out.clone()
            # mask_out_c[mask_out <= 0.4] = 0
            # mask_out_c[mask_out > 0.4] = 1
            self.total_loss += 0.5 * self.l1_loss(mask_out * left_out + (1 - mask_out) * right_out,
                                                  original_image_torch)
            # self.current_gradient = self.gray_loss(mask_out)
            # # self.current_gradient = self.gradient_loss(mask_out)
            # iteration = min(iteration, 1000)
            # self.total_loss += (0.001 * (iteration // 100)) * self.current_gradient  #0.001
        self.total_loss.backward(retain_graph=True)
    ...
    def _finalize_iteration(self):
        ...
        mask_out_np = torch_to_np(self.fg_hints_torch[0])
...
def segmentation_nomask(image_name, image, fore, back, save_path):
    results = []
    for i in range(1):
        s = Segmentation(image_name+"_{}".format(i), image, fg_hint=fore, bg_hint=back, save_path=save_path)
        s.optimize()
        #s.finalize()
        results.append(s.best_result)

    save_image(image_name + "_left", median([best_result.left for best_result in results]), save_path)
    save_image(image_name + "_right", median([best_result.right for best_result in results]), save_path)
    save_image(image_name + "_original", image, save_path)
    # save_image(image_name + "_mask", median([best_result.mask for best_result in results]), save_path)
    # save_image(image_name + "_learned_mask", median([best_result.learned_mask for best_result in results]), save_path)
    # recovered_mask = median([best_result.learned_mask for best_result in results])
    # clear_image_places = np.zeros_like(recovered_mask)
    # clear_image_places[recovered_mask > 0.4] = 1
    save_image(image_name + "_real_left", fore * median([best_result.left for best_result in results]), save_path)
    save_image(image_name + "_real_right", image * back + fore * median([best_result.right for best_result in results]), save_path)
